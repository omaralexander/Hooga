...
//  Hooga
//
//  Created by Omar Abbas on 12/19/16.
//  Copyright © 2016 Omar Abbas. All rights reserved.
//
import UIKit
import Firebase

class MessagesController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    override func viewDidLoad() {
        ...
        username.text = messageUser?.name

        profilepicture.loadImageUsingCachWithUrlString(urlString: (messageUser?.profileImageUrl)!)
        
        sendMessage.addTarget(self, action: #selector(handleSend), for: .touchUpInside)
        
        sendPhoto.addTarget(self, action:#selector(self.handleUploadTap), for: .touchUpInside)
        ...
        self.profilepicture.layer.cornerRadius =
            profilepicture.frame.size.width / 2;
    ...
    func handleSend(){
        ...
        let fromId = FIRAuth.auth()!.currentUser!.uid
        let timestamp = NSDate().timeIntervalSince1970
        let values = ["text": self.messageTextField.text!,"toId": toId,"fromId": fromId,"timeStamp":timestamp] as [String : Any]
    ...
    let viewController = segue.destination as! loadingUserMessages
