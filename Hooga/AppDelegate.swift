...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let gcmMessageText = "gcm.message_text"
        ...
        STPPaymentConfiguration.shared().publishableKey = "pk_test_km37l0MBOQNqujXiKaaieQtT"
    ...
    func connectToFcm() {
        ...
        FIRMessaging.messaging().connect { (error) in
            if error != nil {
                ...
                let refreshedTokenUpHere = FIRInstanceID.instanceID().token()
                
                let uid = FIRAuth.auth()?.currentUser?.uid
                if uid == nil {
                    print("no user in here")
                }else{
                    FIRDatabase.database().reference().child("Users").child(uid!).updateChildValues(["token":refreshedTokenUpHere! as String])
